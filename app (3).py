...
<style>
    ...
    .participant-block {{
        background: {LIGHT_GRAY}; border-radius: 16px; padding: 20px 24px; margin-bottom: 16px;
...
if 'participants' not in st.session_state:
    # Список участников: каждый элемент — dict с ключами inn, price
...
for i, participant in enumerate(st.session_state.participants):
    st.markdown(f'<div class="participant-block">', unsafe_allow_html=True)
...
def fetch_kontur_data(inn, api_key):
    ...
    try:
        ...
        # 4. /api3/briefReport - Желтые/красные записи
...
def calculate_scores(df):
    ...
    # 4. Отсутствие желтых/красных записей
