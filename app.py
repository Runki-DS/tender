...
def fetch_kontur_data(inn, api_key):
    ...
    try:
        ...
        # 2. /api3/accountingReports — Выручка (код 2110 последнего года)
        ...
        # 3. /api3/legalAnalytics — Штат (staffNumber на последнюю дату)
        ...
        if r.status_code == 200 and r.json():
            ...
            if data and len(data) > 0:
                ...
                # Основной вариант — список записей под ключом staffNumber
                sn = analytics.get('staffNumber')
                if isinstance(sn, list) and sn:
                    staff_records = sn
                elif isinstance(sn, dict):
                    staff_records = [sn]

                # Fallback: альтернативные ключи
                if not staff_records:
                    for key in ('staff', 'staffNumbers', 'employees', 'employeeCount'):
                        block = analytics.get(key)
                ...
                def _get_date(rec):
                    return rec.get('date') or rec.get('actualDate') or rec.get('period') or ''

                if staff_records:
                    staff_records = sorted(staff_records, key=_get_date)
                    last_rec = staff_records[-1]
                    value = (
                        last_rec.get('staffNumber')
                        or last_rec.get('count')
                        or last_rec.get('value')
                        or 0
                    )
                    try:
                        result['employees'] = int(value)
                    except (TypeError, ValueError):
                        result['employees'] = 0

        # 4. /api3/briefReport — Жёлтые и красные маркеры из блока summary
        ...
        if r.status_code == 200 and r.json():
            ...
            yellow = summary.get('yellowStatements', []) or []
            red = summary.get('redStatements', []) or []
            ...
            for y in yellow:
                if isinstance(y, dict):
                    result['risk_text'].append(f"⚠️ {y.get('text', y.get('description', str(y)))}")
                else:
                    result['risk_text'].append(f"⚠️ {y}")

            for rd in red:
                if isinstance(rd, dict):
                    result['risk_text'].append(f"🔴 {rd.get('text', rd.get('description', str(rd)))}")
                else:
                    result['risk_text'].append(f"🔴 {rd}")

        # 5. /api3/scoring — Сумма rating по трём целевым моделям
        SCORING_MODEL_IDS = {
        ...
        if r.status_code == 200 and r.json():
            ...
            for entry in scoring_list:
                models = entry.get('scoring', []) if isinstance(entry, dict) else []
                for model in models:
                    if model.get('modelId') in SCORING_MODEL_IDS:
        ...
        # 6-8. Расчёты из бухотчётности (аванс, оборачиваемость, чистые активы)
        if buh_forms:
            years = sorted([f.get('year') for f in buh_forms if f.get('year')], reverse=True)
...
if st.session_state.analysis_result is not None:
    ...
    display_df.columns = ['Компания', 'ИНН', 'Цена', 'Выручка', 'Штат',
                          'Скоринг (сумма)', 'Аванс', 'Оборач.', 'Чистые активы', 'Жёлтых', 'Красных', 'Баллы']
