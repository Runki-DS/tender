...
def fetch_kontur_data(inn, api_key):
    ...
    SCORING_MODEL_IDS = {
        "304a90a4-8882-4be8-9c01-72e2b356179d",  # Критерии налоговой проверки
        "7f5311e1-e7c0-4535-a85b-4bb2eebcee79",  # Критерии благонадёжности
        "3e64aa23-2653-412d-b072-c04a948e6402",  # Финансовое состояние
    }
    ...
    try:
        ...
        # 3. /api3/legalAnalytics — Штат из legalAnalyticsData.staffNumber
        try:
        ...
        # 4. /api3/briefReport — флаги + ссылка на полный отчёт
        ...
        # 5. /api3/scoring — сумма rating по целевым моделям + сбор Risk-маркеров
        try:
        ...
        # Fallback, если флаги есть, а маркеров нет
...
def calculate_scores(df):
    ...
    # 1. Доля тендера к выручке (наименьшая)
    ...
    # 3. Превышение аванса к 10% от тендера
    ...
    # 4. Нет жёлтых и красных записей
...
def render_card(row, position_label, css_class, badge_class):
    """Отрисовывает карточку участника. HTML собирается в одну строку без отступов."""
    risk_html = ""
    if row['risk_text']:
